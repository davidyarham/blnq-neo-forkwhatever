...
function setActiveTab(tab, groupSelector) {
	document.querySelectorAll(groupSelector).forEach(t => {
...
const CARD_GRADIENTS = [
	'linear-gradient(135deg, #db0011 0%, #8b0008 60%, #c20010 100%)',
	'linear-gradient(135deg, #1a3a52 0%, #0f1f2e 60%, #16324a 100%)',
	'linear-gradient(135deg, #0066cc 0%, #003d99 60%, #0055bb 100%)',
	'linear-gradient(135deg, #1a4731 0%, #0d2b1d 60%, #166534 100%)',
	'linear-gradient(135deg, #4a1a6b 0%, #2d0f42 60%, #6b21a8 100%)',
	'linear-gradient(135deg, #7c2d12 0%, #431407 60%, #9a3412 100%)',
	'linear-gradient(135deg, #0f4c5c 0%, #082f3a 60%, #0e7490 100%)',
	'linear-gradient(135deg, #1e3a5f 0%, #0f2440 60%, #1d4ed8 100%)',
];
...
function renderCards() {
	...
	container.innerHTML = dummyCards.map(card => `
		<div class="card-item ${card.status === 'frozen' ? 'frozen' : ''}" style="background: ${card.gradient}">
			<div class="card-item__shine"></div>
			<div>
	...
	// Shine effect on each card
	document.querySelectorAll('.card-item').forEach(el => {
		const shine = el.querySelector('.card-item__shine');
		el.addEventListener('mousemove', e => {
			const b = el.getBoundingClientRect();
			const dx = e.clientX - b.left - b.width / 2;
			const dy = e.clientY - b.top - b.height / 2;
			const relX = ((e.clientX - b.left) / b.width) * 100;
			const relY = ((e.clientY - b.top) / b.height) * 100;
			el.style.setProperty('--mouse-x', `${relX}%`);
			el.style.setProperty('--mouse-y', `${relY}%`);
			shine.style.setProperty('--shine-x', `${relX}%`);
			shine.style.setProperty('--shine-y', `${relY}%`);
			const angle = Math.atan2(dy, dx) * (180 / Math.PI) + 135;
			shine.style.setProperty('--angle', `${angle}deg`);
		});
		el.addEventListener('mouseleave', () => {
			el.style.setProperty('--mouse-x', '50%');
			el.style.setProperty('--mouse-y', '50%');
			shine.style.setProperty('--shine-x', '50%');
			shine.style.setProperty('--shine-y', '30%');
		});
	});
}
...
function bindPanelControls() {
	...
	document.getElementById('slide-panel')?.addEventListener('click', e => {
		if (e.target.closest('[id^="close-panel"], [id^="sm-cancel"]')) {
			closePanel();
		}
	});
...
function initCarousel(selector = '.carousel') {
	...
	const markers = carousel.querySelectorAll(`${selector}__marker`);

	if (!track || slides.length === 0 || markers.length === 0) return;
	...
	function updateMarkers(index) {
		markers.forEach((m, i) => {
			m.classList.toggle(`${selector.substring(1)}__marker--active`, i === index);
		});
	}
	...
	markers.forEach((marker, i) => {
		marker.addEventListener('click', () => scrollToSlide(i));
	});

	track.addEventListener('scroll', () => {
		updateMarkers(getActiveIndex());
	}, { passive: true });
...
function bindMapToggle() {
	document.querySelectorAll('[data-map-type]').forEach(btn => {
		btn.addEventListener('click', () => {
			...
			setActiveTab(btn, '[data-map-type]');
...
function initFXCards() {
	...
	waitForChart(() => {
		...
		// Pause / resume FX rate streaming
		var fxPaused = false;
		var fxPauseBtn = document.getElementById('fx-pause-btn');
		if (fxPauseBtn) {
			fxPauseBtn.addEventListener('click', function () {
				fxPaused = !fxPaused;
				fxPauseBtn.setAttribute('aria-pressed', fxPaused);
				fxPauseBtn.setAttribute('aria-label', fxPaused ? 'Resume FX rate streaming' : 'Pause FX rate streaming');
				var icon = fxPauseBtn.querySelector('i');
				var label = fxPauseBtn.querySelector('.fx-pause-btn__label');
				if (icon) icon.setAttribute('data-lucide', fxPaused ? 'play' : 'pause');
				if (label) label.textContent = fxPaused ? 'Resume' : 'Pause';
				fxPauseBtn.classList.toggle('fx-pause-btn--paused', fxPaused);
				var liveBadge = document.getElementById('fx-live-badge');
				if (liveBadge) liveBadge.classList.toggle('live-badge--paused', fxPaused);
				refreshIcons();
			});
		}

		// Update FX rates every second
...
function navigateTo(page) {
	...
	document.querySelectorAll('.mobile-nav__item[data-page]').forEach(n => {
...
function bindNavigation() {
	...
	document.querySelectorAll('.mobile-nav__item[data-page]').forEach(btn => {
		btn.addEventListener('click', () => navigateTo(btn.dataset.page));
	});
}
...
function initSpendingChart(range = '1y') {
	...
	const all1y = {
		...
		current: [48500, 46200, 44800, 47300, 43900, 46700, 45200, 42800, 38600, 41500, 46900, 52100]
	};
	let labels, current;
	if (range === '1m') {
		...
		labels = all6m.labels.slice(3);
		current = all6m.current.slice(3);
	} else if (range === '6m') {
		labels = all6m.labels;
		current = all6m.current;
	} else {
		labels = all1y.labels;
		current = all1y.current;
	}
...
function bindChartToggles() {
	document.querySelectorAll('.tab[data-range]').forEach(btn => btn.addEventListener('click', () => {
		setActiveTab(btn, '.tab[data-range]');
...
function init() {
	...
	initCarousel('.acct-summary-carousel');
...
(function initSuperPayment() {
	const panel = document.getElementById('super-payment-panel');
	if (!panel) return;

	// ── Payee pill selection ──────────────────
	const pills = panel.querySelectorAll('.sp-pill');
	pills.forEach(pill => {
		pill.addEventListener('click', () => {
			...
			// Fill search input with payee name
			const searchInput = document.getElementById('sp-payee-input');
			if (searchInput) {
				searchInput.value = pill.dataset.payee;
			}
		});
	});

	// ── Payee search filter ───────────────────
	const payeeInput = document.getElementById('sp-payee-input');
	if (payeeInput) {
		payeeInput.addEventListener('input', () => {
			const q = payeeInput.value.toLowerCase().trim();
			pills.forEach(pill => {
				const name = pill.dataset.payee.toLowerCase();
	...
	// ── Add payee button ──────────────────────
	const addPayeeBtn = panel.querySelector('.sp-add-payee-btn');
	if (addPayeeBtn) {
		addPayeeBtn.addEventListener('click', () => {
			const rawVal = payeeInput ? payeeInput.value.trim() : '';
			const name = rawVal || 'New Payee';
			...
			const pillsContainer = document.getElementById('sp-payee-pills');
			...
			newPill.className = 'sp-pill sp-pill--active sp-pill--new';
			newPill.type = 'button';
			newPill.role = 'listitem';
			newPill.dataset.payee = name;
			newPill.setAttribute('aria-pressed', 'true');
			newPill.innerHTML = `<span class="sp-pill__avatar">${initials}</span>${name}`;
			newPill.addEventListener('click', () => {
				panel.querySelectorAll('.sp-pill').forEach(p => {
					p.classList.remove('sp-pill--active');
					p.setAttribute('aria-pressed', 'false');
				});
				newPill.classList.add('sp-pill--active');
				newPill.setAttribute('aria-pressed', 'true');
				if (payeeInput) payeeInput.value = newPill.dataset.payee;
			});
	...
	// ── Currency dropdown ─────────────────────
	const currencyBtn  = document.getElementById('sp-currency-btn');
	const currencyDrop = document.getElementById('sp-currency-dropdown');
	const currencyFlag = document.getElementById('sp-currency-flag');
	const currencyCode = document.getElementById('sp-currency-code');

	function openCurrencyDrop() {
		if (!currencyDrop) return;
		currencyDrop.hidden = false;
		currencyBtn.setAttribute('aria-expanded', 'true');
	}

	function closeCurrencyDrop() {
		if (!currencyDrop) return;
		currencyDrop.hidden = true;
	...
	if (currencyBtn) {
		currencyBtn.addEventListener('click', e => {
			e.stopPropagation();
			currencyDrop.hidden ? openCurrencyDrop() : closeCurrencyDrop();
		});
	}

	if (currencyDrop) {
		currencyDrop.querySelectorAll('.sp-currency-option').forEach(opt => {
			opt.addEventListener('click', () => {
				// Update button
				currencyFlag.textContent = opt.dataset.flag;
				currencyCode.textContent = opt.dataset.code;

				// Update selected state
				currencyDrop.querySelectorAll('.sp-currency-option').forEach(o => {
					o.classList.remove('sp-currency-option--selected');
					o.setAttribute('aria-selected', 'false');
				});
				opt.classList.add('sp-currency-option--selected');
	...
	document.addEventListener('click', e => {
		if (currencyDrop && !currencyDrop.hidden && !panel.contains(e.target)) {
			closeCurrencyDrop();
		}
	});

	// ── Footer buttons ────────────────────────
	const payBtn      = document.getElementById('sp-pay-btn');
	const scheduleBtn = document.getElementById('sp-schedule-btn');

	if (payBtn) {
		payBtn.addEventListener('click', () => {
			const selectedPill = panel.querySelector('.sp-pill--active');
			const payee  = selectedPill ? selectedPill.dataset.payee : null;
			const amount = document.getElementById('sp-amount-input')?.value;
			const code   = currencyCode?.textContent || 'GBP';

			if (!payee) { alert('Please select a payee first.'); return; }
			if (!amount || parseFloat(amount) <= 0) { alert('Please enter a valid amount.'); return; }

			alert(`✓ Payment of ${code} ${parseFloat(amount).toFixed(2)} to ${payee} submitted!`);
		});
	}
	...
	if (scheduleBtn) {
		scheduleBtn.addEventListener('click', () => {
			...
			alert(`📅 Payment of ${code} ${parseFloat(amount).toFixed(2)} to ${payee} has been scheduled.`);
		});
	}
})();
